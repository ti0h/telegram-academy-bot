...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    intro = (
        "<i>— Приветствую. Какими судьбами в Академии Пафент? Если пришёл без дела — не смею задерживать. Если же оно у тебя всё-таки есть, говори. Возможно, я даже соизволю выслушать. А если настроение окажется достаточно хорошим... кто знает, может, и помогу.</i>\n\n"
        "<i>— Хочешь создать анкету? Любопытно. Посмотрим, кем ты станешь в моём мире. Очередным студентом, который мечтает постичь тайны магии, или человеком, способным принести Академии хоть какую-то пользу? Выбирай роль, а дальше постарайся не разочаровать ни меня, ни анкетологов.</i>\n\n"
        "<i>— Итак, кто ты? Ученик, только начинающий свой путь, или будущий сотрудник Академии? Выбирай. Надеюсь, хотя бы с этим трудностей не возникнет.</i>"
    )
...
async def process_choice(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "choice_student":
        sent = await callback.message.answer(
            "<b>Для ученика</b>\n\n"
            "<i>— Начнём с самого простого. Представься. Имя и фамилия. Желательно настоящие — ложь имеет неприятное свойство очень быстро вскрываться.</i>",
            parse_mode="HTML"
        )
        ...
        sent = await callback.message.answer(
            "<b>Для персонала</b>\n\n"
            "<i>— Решил не учиться, а учить других? Любопытно. Назови должность, на которую претендуешь.</i>",
            parse_mode="HTML"
        )
        await state.update_data(last_bot_message_id=sent.message_id)
