...
async def staff_position(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Имя / Фамилия</b>\n\n<i>— Представься. Имя и фамилия. Надеюсь, настоящие.</i>",
...
async def staff_name(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Возраст</b>\n\n<i>— Возраст. Чем старше маг, тем интереснее обычно его история.</i>",
        reply_markup=get_back_keyboard()
    )
...
async def staff_age(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Раса</b>\n\n<i>— Теперь раса. Выбирай из доступных. Не стоит изобретать новую только ради оригинальности.</i>",
...
async def staff_race(callback: types.CallbackQuery, state: FSMContext):
    ...
    sent2 = await callback.message.answer(
        "<b>Пол / Рост / Вес</b>\n\n<i>— Пол, рост и вес. Даже такие мелочи имеют значение.</i>",
        reply_markup=get_back_keyboard(),
        parse_mode="HTML"
    )
...
async def staff_gender_height_weight(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Характер</b>\n\n<i>— Опиши характер. Не двумя строчками о том, какой ты замечательный, а так, чтобы было понятно, с кем предстоит работать.</i>\n\n<b>Минимум 200 символов.</b>",
        reply_markup=get_back_keyboard()
    )
...
async def staff_character(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Способности, магия и магическое направление</b>\n\n<i>— Теперь способности. Опиши магию, её направление и особенности. Напоминаю: всесилие, бессмертие, антимагия, техномагия, телепортация, контроль людей и существ — запрещены. Исключений не существует.</i>",
        reply_markup=get_back_keyboard()
    )
...
async def staff_abilities(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Слабости, страхи</b>\n\n<i>— Любая сила имеет предел. Расскажи о слабых сторонах своих способностей и страхах. Именно они делают персонажа живым.</i>",
        reply_markup=get_back_keyboard()
    )
...
async def staff_weaknesses(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Факты</b>\n\n<i>— Напиши несколько интересных фактов о себе. Здесь же укажи особые состояния персонажа: вампиризм, ликантропию, проклятия, одержимость и любые другие особенности.</i>",
        reply_markup=get_back_keyboard()
    )
...
async def staff_facts(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Внешность</b>\n\n<i>— Опиши внешность. Если есть изображение — кратко. Если нет — подробно. Преподавателя должны узнавать не только по имени.</i>",
        reply_markup=get_back_keyboard()
    )
...
async def staff_appearance(message: types.Message, state: FSMContext):
    ...
    await cleanup_and_send(
        message,
        state,
        "<b>Биография</b>\n\n<i>— Биография. Не забывай, что преподавателем Академии можно стать только после прохождения экзамена. Учти это при написании истории.</i>\n\n"
        "<b>Минимум 200 символов.</b>",
        reply_markup=get_back_keyboard()
    )
...
async def staff_biography(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<i>— Теперь остаётся дождаться проверки. На исправление замечаний даётся столько времени, сколько пожелаешь. просто отправь отредактированную анкету заново через команду /start И ещё: бронь на персонажа действует одну неделю. Не заставляй её истекать впустую.</i>",
        parse_mode="HTML"
    )
